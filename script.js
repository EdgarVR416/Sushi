...
function initCurrencyTabs() {
    ...
    const defaultTab = document.querySelector('.currency-tab[data-currency="robux"]');
    if (defaultTab) {
        defaultTab.classList.add('active');
    }
    
    currencyPrices.forEach(price => {
        if (price.classList.contains('robux')) {
...
function toggleFAQ(index) {
    ...
    allAnswers.forEach((otherAnswer, otherIndex) => {
        if (otherIndex !== index) {
            ...
            if (allIcons[otherIndex]) {
                allIcons[otherIndex].style.transform = 'rotate(0deg)';
            }
            
            if (allQuestions[otherIndex]) {
                allQuestions[otherIndex].style.backgroundColor = 'transparent';
            }
        }
    });
    ...
    if (answer.classList.contains('hidden')) {
        ...
        icon.style.transform = 'rotate(180deg)';
        question.style.backgroundColor = 'rgba(55, 65, 81, 0.3)';
